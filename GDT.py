...
if FreeCAD.GuiUp:
    ...
    FreeCAD.Console.PrintMessage(
        "FreeCAD Gui not present. GDT module will have some features disabled."
    )
    gui = True
...
try:
    ...
    FreeCAD.Console.PrintMessage(
        "Error: Python-pyside package must be installed on your system to use the Geometric Dimensioning & Tolerancing module."
    )

__dir__ = os.path.dirname(__file__)
iconPath = os.path.join(__dir__, "Gui", "Resources", "icons")
path_dd_resources = os.path.join(
    os.path.dirname(__file__), "Gui", "Resources", "dd_resources.rcc"
)
...
def getAllGDTObjects():
    ...
    return getObjectsOfType(
        ["AnnotationPlane", "DatumFeature", "DatumSystem", "GeometricTolerance"]
    )
...
def makeContainerOfData():
    ...
    for i in range(len(getSelectionEx())):
        for j in range(len(getSelectionEx()[i].SubElementNames)):
            faces.append(
                (getSelectionEx()[i].Object, getSelectionEx()[i].SubElementNames[j])
            )
    faces.sort()
...
def getPointsToPlot(obj):
    ...
    if obj.GT != [] or obj.DF != None:
        ...
        existGT = True
        if obj.GT != []:
            points, segments = getPointsToPlotGT(
                obj, points, segments, Vertical, Horizontal
            )
        else:
            existGT = False
        if obj.DF != None:
            points, segments = getPointsToPlotDF(
                obj, existGT, points, segments, Vertical, Horizontal
            )
        segments = segments + []
...
def getPointsToPlotGT(obj, points, segments, Vertical, Horizontal):
    ...
    for i in range(len(obj.GT)):
        ...
        if points[2].x < points[0].x:
            P0 = (
                newPoints[-1] + Vertical * (sizeOfLine)
                if i == 0
                else FreeCAD.Vector(newPoints[-2])
            )
        else:
            P0 = (
                newPoints[-1] + Vertical * (sizeOfLine)
                if i == 0
                else FreeCAD.Vector(newPoints[-1])
            )
        P1 = P0 + Vertical * (-sizeOfLine * 2)
        P2 = P0 + Horizontal * (sizeOfLine * 2)
        P3 = P1 + Horizontal * (sizeOfLine * 2)
        lengthToleranceValue = len(
            stringencodecoin(
                displayExternal(
                    obj.GT[i].ToleranceValue,
...
def plotStrings(self, fp, points):
    ...
    if fp.GT != []:
        for i in range(len(fp.GT)):
            ...
            self.points[indexIcon].point.setValues(
                [
                    [auxPoint.x, auxPoint.y, auxPoint.z],
                    [
                        points[5 + displacement].x,
                        points[5 + displacement].y,
                        points[5 + displacement].z,
                    ],
                    [
                        points[4 + displacement].x,
                        points[4 + displacement].y,
                        points[4 + displacement].z,
                    ],
                    [
                        points[3 + displacement].x,
                        points[3 + displacement].y,
                        points[3 + displacement].z,
                    ],
                ]
            )
            ...
            displacementH = ((Horizontal * auxPoint) % (sizeOfLine * 2)) / (
                sizeOfLine * 2
            )
            displacementV = ((Vertical * auxPoint) % (sizeOfLine * 2)) / (
                sizeOfLine * 2
            )
            self.textureTransform[indexIcon].translation.setValue(
                -displacementH, -displacementV
            )
            ...
            if fp.GT[i].FeatureControlFrameIcon != "":
                ...
                self.points[indexIcon].point.setValues(
                    [
                        [auxPoint1.x, auxPoint1.y, auxPoint1.z],
                        [
                            points[7 + displacement].x,
                            points[7 + displacement].y,
                            points[7 + displacement].z,
                        ],
                        [
                            points[6 + displacement].x,
                            points[6 + displacement].y,
                            points[6 + displacement].z,
                        ],
                        [auxPoint2.x, auxPoint2.y, auxPoint2.z],
                    ]
                )
                self.face[indexIcon].numVertices = 4
                self.svgPos[indexIcon].directionS.setValue(dS.x, dS.y, dS.z)
                self.svgPos[indexIcon].directionT.setValue(dT.x, dT.y, dT.z)
                displacementH = ((Horizontal * auxPoint1) % (sizeOfLine * 2)) / (
                    sizeOfLine * 2
                )
                displacementV = ((Vertical * auxPoint1) % (sizeOfLine * 2)) / (
                    sizeOfLine * 2
                )
                self.textureTransform[indexIcon].translation.setValue(
                    -displacementH, -displacementV
            ...
            if fp.GT[i].Circumference:
                ...
                self.svgPos[indexIcon].directionT.setValue(dT.x, dT.y, dT.z)
                displacementH = (
                    (Horizontal * points[5 + displacement]) % (sizeOfLine * 2)
                ) / (sizeOfLine * 2)
                displacementV = (
                    (Vertical * points[5 + displacement]) % (sizeOfLine * 2)
                ) / (sizeOfLine * 2)
                self.textureTransform[indexIcon].translation.setValue(
                    -displacementH, -displacementV
                )
            ...
            self.textGT[index].string = self.textGT3d[index].string = stringencodecoin(
                displayExternal(
                    fp.GT[i].ToleranceValue,
                    fp.ViewObject.Decimals,
                    "Length",
                    fp.ViewObject.ShowUnit,
                )
            )
            self.textGTpos[index].translation.setValue(
                [posToleranceValue.x, posToleranceValue.y, posToleranceValue.z]
            )
            ...
            if fp.GT[i].DS != None and fp.GT[i].DS.Primary != None:
                ...
                self.textGT[index].string = self.textGT3d[index].string = str(
                    fp.GT[i].DS.Primary.Label
                )
                self.textGTpos[index].translation.setValue(
                    [posPrimary.x, posPrimary.y, posPrimary.z]
                )
                self.textGT[index].justification = coin.SoAsciiText.CENTER
                index += 1
                displacement += 2
                if fp.GT[i].DS.Secondary != None:
                    ...
                    self.textGT[index].string = self.textGT3d[index].string = str(
                        fp.GT[i].DS.Secondary.Label
                    )
                    self.textGTpos[index].translation.setValue(
                        [posSecondary.x, posSecondary.y, posSecondary.z]
                    )
                    self.textGT[index].justification = coin.SoAsciiText.CENTER
                    index += 1
                    displacement += 2
                    if fp.GT[i].DS.Tertiary != None:
                        ...
                        self.textGT[index].string = self.textGT3d[index].string = str(
                            fp.GT[i].DS.Tertiary.Label
                        )
                        self.textGTpos[index].translation.setValue(
                            [posTertiary.x, posTertiary.y, posTertiary.z]
                        )
                        self.textGT[index].justification = coin.SoAsciiText.CENTER
                        index += 1
                        displacement += 2
        if fp.circumferenceBool and True in [l.Circumference for l in fp.GT]:
            ...
            self.points[indexIcon].point.setValues(
                [
                    [auxPoint1.x, auxPoint1.y, auxPoint1.z],
                    [auxPoint2.x, auxPoint2.y, auxPoint2.z],
                    [auxPoint3.x, auxPoint3.y, auxPoint3.z],
                    [auxPoint4.x, auxPoint4.y, auxPoint4.z],
                ]
            )
            self.face[indexIcon].numVertices = 4
            self.svgPos[indexIcon].directionS.setValue(dS.x, dS.y, dS.z)
            ...
            self.textGTpos[index].translation.setValue(
                [posDiameterTolerance.x, posDiameterTolerance.y, posDiameterTolerance.z]
            )
            if fp.toleranceSelectBool:
                text = stringencodecoin(
                    displayExternal(
                        fp.diameter,
                        fp.ViewObject.Decimals,
                        "Length",
    ...
    if fp.DF != None:
        ...
        self.textDFpos.translation.setValue(
    ...
    if fp.GT != [] or fp.DF != None:
        if len(fp.faces) > 1:
            ...
            self.textGT[index].string = self.textGT3d[index].string = (
                str(len(fp.faces)) + "x"
            )
            self.textGTpos[index].translation.setValue(
                [posNumFaces.x, posNumFaces.y, posNumFaces.z]
            )
            self.textGT[index].justification = coin.SoAsciiText.CENTER
            try:
                DirectionAux = FreeCAD.Vector(fp.AP.Direction)
...
class _AnnotationPlane(_GDTObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty(
            ...
            "Offset",
            "GDT",
            "The offset value to aply in this annotation plane",
        )
        obj.addProperty(
            "App::PropertyLinkSub", "faces", "GDT", "Linked face of the object"
        ).faces = (getSelectionEx()[0].Object, getSelectionEx()[0].SubElementNames[0])
        obj.addProperty(
            "App::PropertyVectorDistance", "p1", "GDT", "Center point of Grid"
        ).p1 = (obj.faces[0].Shape.getElement(obj.faces[1][0]).CenterOfMass)
        obj.addProperty(
            "App::PropertyVector",
            "Direction",
            "GDT",
            "The normal direction of this annotation plane",
        ).Direction = (
        ...
        obj.addProperty(
            ...
            "PointWithOffset",
            "GDT",
            "Center point of Grid with offset applied",
        )
        ...
        if hasattr(FreeCADGui, "Snapper"):
            if FreeCADGui.Snapper.grid:
                FreeCAD.DraftWorkingPlane.alignToPointAndAxis(
                    self.Object.PointWithOffset, self.Object.Direction, 0
                )
...
def makeAnnotationPlane(Name, Offset):
    ...
    if len(getAllAnnotationPlaneObjects()) == 0:
        group = FreeCAD.ActiveDocument.addObject(
            "App::DocumentObjectGroupPython", "GDT"
        )
        _GDTObject(group)
...
def makeDatumFeature(Name, ContainerOfData):
    ...
    if AnnotationObj == None:
        makeAnnotation(
            ContainerOfData.faces, ContainerOfData.annotationPlane, DF=obj, GT=[]
        )
    else:
        ...
        group = makeAnnotation(
            ...
            toleranceDiameter=toleranceDiameter,
            lowLimit=lowLimit,
            highLimit=highLimit,
        )
        group.addObject(obj)
...
class _DatumSystem(_GDTObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty(
            "App::PropertyLink", "Primary", "GDT", "Primary datum feature used"
        )
        obj.addProperty(
            "App::PropertyLink", "Secondary", "GDT", "Secondary datum feature used"
        )
        obj.addProperty(
            "App::PropertyLink", "Tertiary", "GDT", "Tertiary datum feature used"
        )
...
class _GeometricTolerance(_GDTObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty(
            ...
            "Characteristic",
            "GDT",
            "Characteristic of the geometric tolerance",
        )
        obj.addProperty(
            "App::PropertyString",
            "CharacteristicIcon",
            "GDT",
            "Characteristic icon path of the geometric tolerance",
        )
        obj.addProperty(
            "App::PropertyBool",
            "Circumference",
            "GDT",
            "Indicates whether the tolerance applies to a given diameter",
        )
        obj.addProperty(
            "App::PropertyFloat",
            "ToleranceValue",
            "GDT",
            "Tolerance value of the geometric tolerance",
        )
        obj.addProperty(
            "App::PropertyString",
            "FeatureControlFrame",
            "GDT",
            "Feature control frame of the geometric tolerance",
        )
        obj.addProperty(
            "App::PropertyString",
            "FeatureControlFrameIcon",
            "GDT",
            "Feature control frame icon path of the geometric tolerance",
        )
...
def makeGeometricTolerance(Name, ContainerOfData):
    ...
    if AnnotationObj == None:
        makeAnnotation(
            ContainerOfData.faces,
            ContainerOfData.annotationPlane,
            DF=None,
            GT=obj,
...
class _Annotation(_GDTObject):
    ...
    def __init__(self, obj):
        ...
        obj.addProperty(
            "App::PropertyLinkSubList", "faces", "GDT", "Linked faces of the object"
        )
        ...
        obj.addProperty(
            ...
            "Direction",
            "GDT",
            "The normal direction of your annotation plane",
        )
        obj.addProperty(
            "App::PropertyVector",
            "selectedPoint",
            "GDT",
            "Selected point to where plot the annotation",
        )
        obj.addProperty(
            "App::PropertyBool",
            "spBool",
            "GDT",
            "Boolean to confirm that a selected point exists",
        ).spBool = False
        obj.addProperty(
            "App::PropertyBool",
            "circumferenceBool",
            "GDT",
            "Boolean to determine if this annotation is over a circumference",
        ).circumferenceBool = False
        obj.addProperty("App::PropertyFloat", "diameter", "GDT", "Diameter")
        obj.addProperty(
            "App::PropertyBool",
            "toleranceSelectBool",
            "GDT",
            "Determinates if use plus-minus or low and high limits",
        ).toleranceSelectBool = True
        obj.addProperty(
            "App::PropertyFloat",
            "toleranceDiameter",
            "GDT",
            "Diameter tolerance (Plus-minus)",
        )
        obj.addProperty(
            "App::PropertyFloat", "lowLimit", "GDT", "Low limit diameter tolerance"
        )
        obj.addProperty(
            "App::PropertyFloat", "highLimit", "GDT", "High limit diameter tolerance"
        )
    ...
    def execute(self, fp):
        ...
        if fp.circumferenceBool:
            ...
            fp.p1 = (
                vertexex[0].Point
                if vertexex[0].Point.z > vertexex[1].Point.z
                else vertexex[1].Point
            )
            fp.Direction = fp.AP.Direction
        else:
            fp.p1 = (
                fp.faces[0][0].Shape.getElement(fp.faces[0][1]).CenterOfMass
            ).projectToPlane(fp.AP.PointWithOffset, fp.AP.Direction)
            fp.Direction = (
                fp.faces[0][0].Shape.getElement(fp.faces[0][1]).normalAt(0, 0)
            )
        diff = fp.p1 - auxP1
        if fp.spBool:
...
class _ViewProviderAnnotation(_ViewProviderGDT):
    ...
    def __init__(self, obj):
        obj.addProperty(
            "App::PropertyFloat", "LineWidth", "GDT", "Line width"
        ).LineWidth = getLineWidth()
        obj.addProperty(
            "App::PropertyColor", "LineColor", "GDT", "Line color"
        ).LineColor = getRGBLine()
        obj.addProperty(
            "App::PropertyFloat", "LineScale", "GDT", "Line scale"
        ).LineScale = getParam("lineScale", 1.0)
        obj.addProperty(
            "App::PropertyLength", "FontSize", "GDT", "Font size"
        ).FontSize = getTextSize()
        obj.addProperty(
            "App::PropertyString", "FontName", "GDT", "Font name"
        ).FontName = getTextFamily()
        obj.addProperty(
            "App::PropertyColor", "FontColor", "GDT", "Font color"
        ).FontColor = getRGBText()
        obj.addProperty(
            "App::PropertyInteger", "Decimals", "GDT", "The number of decimals to show"
        ...
        obj.addProperty(
            "App::PropertyBool", "ShowUnit", "GDT", "Show the unit suffix"
    ...
    def attach(self, obj):
        ...
        selectionNode.objectName.setValue(
            obj.Object.Name
        )  # here obj is the ViewObject, we need its associated App Object
        ...
        self.textDF.justification = self.textDF3d.justification = (
            coin.SoAsciiText.CENTER
        )
        ...
        for i in range(20):
            ...
            self.textGT[i].justification = self.textGT3d[i].justification = (
                coin.SoAsciiText.CENTER
            )
    ...
    def updateData(self, fp, prop):
        ...
        # fp is the handled feature, prop is the name of the property that has changed
        if (
            prop in "selectedPoint"
            and hasattr(fp.ViewObject, "Decimals")
            and hasattr(fp.ViewObject, "ShowUnit")
            and fp.spBool
        ):
        ...
        if prop in "faces" and fp.faces != []:
            fp.circumferenceBool = (
                ...
                if (
                    ...
                    and len(fp.faces[0][0].Shape.getElement(fp.faces[0][1]).Vertexes)
                    == 2
                )
                else False
            )
...
def makeAnnotation(
    ...
    Object=None,
    diameter=0.0,
    toleranceSelect=True,
    toleranceDiameter=0.0,
    lowLimit=0.0,
    highLimit=0.0,
):
    """Explanation"""
    if not modify:
        obj = FreeCAD.ActiveDocument.addObject(
            ...
            dictionaryAnnotation[len(getAllAnnotationObjects())],
        )
        _Annotation(obj)
        if gui:
        ...
        if obj.circumferenceBool:
            ...
            index = [l.Point.z for l in vertexex].index(
                max([l.Point.z for l in vertexex])
            )
            obj.p1 = vertexex[index].Point
            obj.Direction = obj.AP.Direction
        else:
            obj.p1 = (
                obj.faces[0][0].Shape.getElement(obj.faces[0][1]).CenterOfMass
            ).projectToPlane(obj.AP.PointWithOffset, obj.AP.Direction)
            obj.Direction = (
                obj.faces[0][0].Shape.getElement(obj.faces[0][1]).normalAt(0, 0)
            )
    else:
        obj = Object
    obj.DF = DF
            ...
            if DF:
                ...
                if checkBoxState:
                    FreeCAD.ActiveDocument.removeObject(
                        getAllDatumSystemObjects()[-1].Name
                    )
            else:
...
def makeCharacteristics(label=None):
    ...
    Icon = [
        ":/dd/icons/Characteristic/straightness.svg",
        ":/dd/icons/Characteristic/flatness.svg",
        ":/dd/icons/Characteristic/circularity.svg",
        ":/dd/icons/Characteristic/cylindricity.svg",
        ":/dd/icons/Characteristic/profileOfALine.svg",
        ":/dd/icons/Characteristic/profileOfASurface.svg",
        ":/dd/icons/Characteristic/perpendicularity.svg",
        ":/dd/icons/Characteristic/angularity.svg",
        ":/dd/icons/Characteristic/parallelism.svg",
        ":/dd/icons/Characteristic/symmetry.svg",
        ":/dd/icons/Characteristic/position.svg",
        ":/dd/icons/Characteristic/concentricity.svg",
        ":/dd/icons/Characteristic/circularRunOut.svg",
        ":/dd/icons/Characteristic/totalRunOut.svg",
    ]
    if label == None:
...
def makeFeatureControlFrame(toolTip=None):
    ...
    Icon = [
        "",
        ":/dd/icons/FeatureControlFrame/freeState.svg",
        ":/dd/icons/FeatureControlFrame/leastMaterialCondition.svg",
        ":/dd/icons/FeatureControlFrame/maximumMaterialCondition.svg",
        ":/dd/icons/FeatureControlFrame/projectedToleranceZone.svg",
        ":/dd/icons/FeatureControlFrame/regardlessOfFeatureSize.svg",
        ":/dd/icons/FeatureControlFrame/tangentPlane.svg",
...
class ContainerOfData(object):
    def __init__(self, faces=[]):
        ...
        if self.faces != []:
            self.Direction = (
                self.faces[0][0].Shape.getElement(self.faces[0][1]).normalAt(0, 0)
            )
            self.DirectionAxis = (
                self.faces[0][0].Shape.getElement(self.faces[0][1]).Surface.Axis
            )
            self.p1 = self.faces[0][0].Shape.getElement(self.faces[0][1]).CenterOfMass
            try:
                edge = [
                    l.Closed
                    for l in self.faces[0][0].Shape.getElement(self.faces[0][1]).Edges
                ].index(True)
                self.diameter = (
                    self.faces[0][0]
                    .Shape.getElement(self.faces[0][1])
                    .Edges[edge]
                    .Length
                    / pi
                )
...
class GDTWidget:
    ...
    def activate(
        self,
        idGDT=0,
        dialogTitle="GD&T Widget",
        dialogIconPath=":/dd/icons/GDT.svg",
        endFunction=None,
        dictionary=None,
    ):
        ...
        if dictionary != None:
            extraWidgets.append(
                textLabelWidget(Text="Name:", Mask="NNNn", Dictionary=self.dictionary)
            )  # http://doc.qt.io/qt-5/qlineedit.html#inputMask-prop
        else:
            extraWidgets.append(textLabelWidget(Text="Name:", Mask="NNNn"))
        self.taskDialog = GDTDialog(
            self.dialogTitle,
            self.dialogIconPath,
            self.idGDT,
            extraWidgets + self.dialogWidgets,
...
class GDTGuiClass(QtGui.QWidget):
    ...
    def createObject(self):
        ...
        if self.idGDT == 1:
            ...
            if checkBoxState:
                makeDatumSystem(
                    auxDictionaryDS[len(getAllDatumSystemObjects())]
                    + ": "
                    + self.textName,
                    obj,
                    None,
                ...
                if self.ContainerOfData.textDS[1] != "":
                    if self.ContainerOfData.textDS[2] != "":
                        self.textName = (
                            self.textName
                            + ": "
                            + separator.join(self.ContainerOfData.textDS)
                        )
                    else:
                        self.textName = (
                            self.textName
                    ...
                    self.textName = (
                        self.textName + ": " + self.ContainerOfData.textDS[0]
                    )
            else:
                self.textName = self.textName
            makeDatumSystem(
                self.textName,
                self.ContainerOfData.primary,
                self.ContainerOfData.secondary,
                self.ContainerOfData.tertiary,
            )
        elif self.idGDT == 3:
...
class fieldLabelWidget:
    ...
    def generateWidget(self, idGDT, ContainerOfData):
        ...
        if hasattr(FreeCADGui, "Snapper"):
            if FreeCADGui.Snapper.grid:
                FreeCAD.DraftWorkingPlane.alignToPointAndAxis(
                    self.ContainerOfData.p1, self.ContainerOfData.Direction, 0.0
                )
                FreeCADGui.Snapper.grid.set()
        ...
        QtCore.QObject.connect(
            self.inputfield, QtCore.SIGNAL("valueChanged(double)"), self.valueChanged
        )
    ...
    def valueChanged(self, d):
        ...
        if hasattr(FreeCADGui, "Snapper"):
            if FreeCADGui.Snapper.grid:
                FreeCAD.DraftWorkingPlane.alignToPointAndAxis(
                    self.ContainerOfData.p1,
                    self.ContainerOfData.Direction,
...
class comboLabelWidget:
    ...
    def generateWidget(self, idGDT, ContainerOfData):
        ...
        for i in range(len(self.List)):
            if self.Icons != None:
                self.ContainerOfData.combo[self.k].addItem(
                    QtGui.QIcon(self.Icons[i]), self.List[i]
                )
            else:
                if self.List[i] == None:
                    self.ContainerOfData.combo[self.k].addItem("")
        ...
        self.ContainerOfData.combo[self.k].activated.connect(
            lambda comboIndex=self.comboIndex: self.updateDate(comboIndex)
        )
    ...
    def updateDate(self, comboIndex):
        ...
        if self.Text == "Primary:":
            self.ContainerOfData.textDS[0] = self.ContainerOfData.combo[
                self.k
            ].currentText()
            ...
            self.ContainerOfData.textDS[1] = self.ContainerOfData.combo[
                self.k
            ].currentText()
            ...
            self.ContainerOfData.textDS[2] = self.ContainerOfData.combo[
                self.k
            ].currentText()
            self.ContainerOfData.tertiary = self.List[comboIndex]
            self.updateItemsEnabled(self.k)
        elif self.Text == "Characteristic:":
            self.ContainerOfData.characteristic = makeCharacteristics(
                self.List[comboIndex]
            )
        elif self.Text == "Datum system:":
            ...
            if hasattr(FreeCADGui, "Snapper"):
                if FreeCADGui.Snapper.grid:
                    FreeCAD.DraftWorkingPlane.alignToPointAndAxis(
                        self.ContainerOfData.PointWithOffset,
                        self.ContainerOfData.Direction,
                        0.0,
                    )
                    FreeCADGui.Snapper.grid.set()
    ...
    def updateItemsEnabled(self, comboIndex):
        ...
        if self.ContainerOfData.combo[comboIndex1].currentIndex() != 0:
            self.ContainerOfData.combo[comboIndex0].model().item(
                self.ContainerOfData.combo[comboIndex1].currentIndex()
            ).setEnabled(False)
        if self.ContainerOfData.combo[comboIndex2].currentIndex() != 0:
            self.ContainerOfData.combo[comboIndex0].model().item(
                self.ContainerOfData.combo[comboIndex2].currentIndex()
            ).setEnabled(False)
        ...
        if self.ContainerOfData.combo[comboIndex0].currentIndex() != 0:
            self.ContainerOfData.combo[comboIndex1].model().item(
                self.ContainerOfData.combo[comboIndex0].currentIndex()
            ).setEnabled(False)
        if self.ContainerOfData.combo[comboIndex2].currentIndex() != 0:
            self.ContainerOfData.combo[comboIndex1].model().item(
                self.ContainerOfData.combo[comboIndex2].currentIndex()
        ...
        if self.ContainerOfData.combo[comboIndex0].currentIndex() != 0:
            self.ContainerOfData.combo[comboIndex2].model().item(
                self.ContainerOfData.combo[comboIndex0].currentIndex()
            ).setEnabled(False)
        if self.ContainerOfData.combo[comboIndex1].currentIndex() != 0:
            self.ContainerOfData.combo[comboIndex2].model().item(
...
class fieldLabeCombolWidget:
    ...
    def generateWidget(self, idGDT, ContainerOfData):
        ...
        self.comboCircumference.setToolTip(
            "Indicates whether the tolerance applies to a given diameter"
        )
        ...
        QtCore.QObject.connect(
            self.inputfieldTolerance,
            QtCore.SIGNAL("valueChanged(double)"),
            self.valueChangedTolerance,
        )
        ...
        if self.Text == "Tolerance value:":
            if self.combo.currentIndex() != 0:
                self.ContainerOfData.featureControlFrame = makeFeatureControlFrame(
                    self.ToolTip[self.combo.currentIndex()]
                )
            else:
                self.ContainerOfData.featureControlFrame = makeFeatureControlFrame("")
